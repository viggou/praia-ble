...
static std::string uuidFromLE(const uint8_t* b, size_t len) {
    char buf[64];
    if (len == 2) {
...
// ── AD record parser ───────────────────────────────────────────────────────
// Walks length-type-value triplets per Core Spec Vol 3 Part C §11.

static Value parseAdImpl(const uint8_t* data, size_t len) {
    auto m = gcNew<PraiaMap>();
        ...
        switch (adType) {
            ...
            case 0x02: case 0x03: // Incomplete/Complete 16-bit Service UUIDs
                for (size_t i = 0; i + 2 <= adDataLen; i += 2)
                    services->elements.push_back(Value(uuidFromLE(adData + i, 2)));
                break;
            case 0x04: case 0x05: // 32-bit
                for (size_t i = 0; i + 4 <= adDataLen; i += 4)
                    services->elements.push_back(Value(uuidFromLE(adData + i, 4)));
                break;
            case 0x06: case 0x07: // 128-bit
                for (size_t i = 0; i + 16 <= adDataLen; i += 16)
                    services->elements.push_back(Value(uuidFromLE(adData + i, 16)));
                break;
            ...
            case 0x16: { // Service Data — 16-bit UUID
                if (adDataLen >= 2) {
                    ...
                    sd->entries["uuid"] = Value(uuidFromLE(adData, 2));
...
extern "C" void praia_register(PraiaMap* module) {
    ...
    module->entries["parseAd"] = Value(makeNative("ble.parseAd", 1,
        [](const std::vector<Value>& args) -> Value {
            ...
            return parseAdImpl(reinterpret_cast<const uint8_t*>(s.data()), s.size());
        }));
}
